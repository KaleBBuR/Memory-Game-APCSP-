...
public class MemoryGameGUI extends JFrame
{
   ...
   * @return integer of the index of which button was chosen! 0-2 Since there are 3 options
   */
  ...
  public int difficulty() {
    // Create an object list to satisfy the showOptionDialog fucntion.
    Object[] difficultyOptions = { "Easy", "Medium", "Hard" };
    // 1) This will make a window with *this* as the frame for our start. Used by other functions as
    // well not sure why to be completely honest. I'm guessing because it incorperates JFrame, since
    // we see the super function when initializing the GUI.
    // 2) Set a description which is "Pick a difficulty"
    // 3) Set a title "Difficulty"
    // 4) Set the option type which is a YES_NO_CANCEL_OPTION, which allows us to have THREE options
    // 5) We choose a question message type, so it makes the box look like a questions
    // 6) null, since no icon
    // 7) Options which we initialized above
    // 8) Set the initial value to be "Easy"
    int pick = JOptionPane.showOptionDialog(this, "Pick a difficulty!", "Difficutly", JOptionPane.YES_NO_CANCEL_OPTION,
